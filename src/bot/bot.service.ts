...
export class BotService {
  ...
  async sendOtp(chatId: number, code: string) {
    console.log("hi");

    const text = `🔐 Login tasdiqlash kodi:\n\n👉 <b>${code}</b>\n\nBu kodni ilovaga kiriting.`;
  ...
  async start(ctx: Context) {
    try {
      ...
      if (!user) {
        ...
        await ctx.reply(
          "Bu bot orqali Bsb tahlili dasturi orqali ro'yhatga o'tilgan",
          {
            parse_mode: 'HTML',
            ...Markup.keyboard([
    ...
    if ('contact' in ctx.message) {
      ...
      if (!user) {
        ...
        if (is_phone) {
          ...
          const url = `https://bsb-front-nu.vercel.app/login`;
          
          await ctx.reply("Siz ro'yhatdan muvaffaqiyatli o'tdingiz!", {
            parse_mode: 'HTML',
            ...Markup.removeKeyboard(),
          });
          await ctx.reply(`[Bsb tahlili saytiga kirish uchun shu yerga bosing](${url})`, { parse_mode: 'MarkdownV2' });
        }
      }
    }
  }
